...
public interface RetirementFundEntryRepository extends JpaRepository<RetirementFundEntry, Long> {
    ...
    @Query("SELECT r.fundType, r.entryType, SUM(r.amount) FROM RetirementFundEntry r WHERE r.userId = :userId GROUP BY r.fundType, r.entryType")
    List<Object[]> summaryByFundAndTypeForUser(@Param("userId") Long userId);
    ...
    List<RetirementFundEntry> findByUserIdAndFundTypeOrderByEntryDateDesc(Long userId, String fundType);
}
